...
function openAvatarPicker() {
    ...
    if (!overlay) return;
    var grid = document.getElementById('ap-avatar-grid');
    var pendingAvatar = userAvatar;

    // pre-select current avatar
    if (grid) {
        grid.querySelectorAll('.setup-avatar-item').forEach(function(el) {
            el.classList.remove('selected');
            if (el.getAttribute('data-src') === userAvatar) el.classList.add('selected');
        });
        // if custom (base64) avatar — nothing in grid gets selected, that's fine
        if (!userAvatar) {
            var def = grid.querySelector('[data-src="avatars/guest.bmp"]');
            if (def) { def.classList.add('selected'); pendingAvatar = 'avatars/guest.bmp'; }
        }
        grid.querySelectorAll('.setup-avatar-item').forEach(function(el) {
            // re-attach clean handler each open
    ...
    // file upload
    var uploadInput = document.getElementById('ap-avatar-upload');
    if (uploadInput) {
        var uploadClone = uploadInput.cloneNode(true);
        uploadInput.parentNode.replaceChild(uploadClone, uploadInput);
        uploadClone.addEventListener('change', function() {
            var file = uploadClone.files[0];
            if (!file) return;
            var reader = new FileReader();
            reader.onload = function(e) {
                pendingAvatar = e.target.result;
                if (grid) grid.querySelectorAll('.setup-avatar-item').forEach(function(el){ el.classList.remove('selected'); });
            };
    ...
    function applyAndClose() {
        if (pendingAvatar) {
            ...
            localStorage.setItem(STORAGE.avatar, userAvatar);
            var smImg = document.getElementById('sm-avatar-img');
            if (smImg) smImg.src = userAvatar;
        }
        overlay.classList.add('hidden');
    }
    function cancelAndClose() { overlay.classList.add('hidden'); }
...
function clippyOpenSolitaire()   { closeStartMenu(); openSolitaire(); }
function clippyOpenPaint()       { closeStartMenu(); openPaint(); }
function clippyOpenHearts()      { closeStartMenu(); openHearts(); }
...
function _clippyMouthExpr(type) {
    ...
    var t = document.getElementById('clippy-tongue');
    if (!m) return;
...
document.querySelectorAll('.sm-pinned-item').forEach(function(el) { el.addEventListener('click', function() { startMenuAction(el.dataset.action); }); });
...
function runStandardInit() {
    ...
    if (smAvatarImg && userAvatar) smAvatarImg.src = userAvatar;
    var smAvatarEl = document.querySelector('.sm-avatar');
    if (smAvatarEl) {
        smAvatarEl.style.cursor = 'pointer';
        smAvatarEl.addEventListener('click', function(e) {
            e.stopPropagation();
            closeStartMenu();
            openAvatarPicker();
        });
...
function initSetupOOBE(onComplete) {
    ...
    if (uploadInput) {
        uploadInput.addEventListener('change', function() {
            ...
            reader.onload = function(e) {
                ...
                if (avatarGrid) avatarGrid.querySelectorAll('.setup-avatar-item').forEach(function(el){ el.classList.remove('selected'); });
                var browseBtn = document.getElementById('setup-avatar-browse-btn');
                if (browseBtn) {
                    browseBtn.style.backgroundImage = 'url(' + userAvatar + ')';
    ...
    nextBtn.addEventListener('click', function() {
        if (currentStep === 1) {
            ...
            if (userAvatar) localStorage.setItem(STORAGE.avatar, userAvatar);
        }
